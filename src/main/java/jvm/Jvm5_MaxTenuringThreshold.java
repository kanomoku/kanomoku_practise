...
public class Jvm5_MaxTenuringThreshold {
	public static void main(String[] args) throws InterruptedException {
		...
		// 由于byte1和byte2的年龄经过3次YoungGC后已经达到3(-XX:MaxTenuringThreshold=3), 所以这次YoungGC后, 会晋升到old
		YoungGC(40);
		// YoungGC后, s0(from)/s1(to)的空间为0
		Thread.sleep(3000);
		...
		// 这次YoungGC时, 由于s区已经占用达到了60%(-XX:TargetSurvivorRatio=60),
		// 所以会重新计算对象晋升的min(age, MaxTenuringThreshold) = 1
		YoungGC(40);
		Thread.sleep(3000);

		// 由于前一次YoungGC时算出age=1, 所以这一次再YoungGC时, byte4, byte5, byte6就要晋升到Old,
		// 而不需要等MaxTenuringThreshold这么多次, 此次YoungGC后, s0(from)/s1(to)的空间再次为0, 对象全部晋升到old
		YoungGC(40);
		Thread.sleep(3000);
	...
	private static void YoungGC(int edenSize) {
