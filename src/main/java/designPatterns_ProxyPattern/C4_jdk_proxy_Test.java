...
//		
		...
		C3_jdk_proxy_Secretary mishu = new C3_jdk_proxy_Secretary();
		//第一个参数：反射时使用的类加载器
		//第二个参数：Proxy需要实现什么接口
		//第三个参数：通过接口对象调用方法时，需要调用哪个类的invoke
		C1_jdk_proxy_Function gongneng = (C1_jdk_proxy_Function) Proxy.newProxyInstance(
				C1_jdk_proxy_Function.class.getClassLoader(),
				new Class[] { C1_jdk_proxy_Function.class }, 
				mishu);
